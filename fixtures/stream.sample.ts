...
export const sampleStream: { delayMs: number; event: StreamEvent }[] = [
  ...
  {
    delayMs: 2_300,
    event: { type: "stage", stage: "fetch", status: "done", counts: { fetched: 61, failed: 6 }, ms: 6_070 },
  },
  {
    delayMs: 90,
    event: { type: "stage", stage: "dedup", status: "done", counts: { kept: 40, duplicates: 21 }, ms: 6_160 },
  },
  ...
  { delayMs: 3_200, event: { type: "photos", photos: samplePhotos.slice(10) } },
  {
    ...
    event: {
      ...
      stage: "verify",
      status: "done",
      counts: { verified: 9, likely: 3, unconfirmed: 2 },
      ms: 14_040,
    },
  },
